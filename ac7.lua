...
local function formatDatabases()
    ...
    if type(saveConfig) == "function" then
        ...
        -- Альтернативная логика или просто игнорируем
        print("Предупреждение: saveConfig не доступна, пропускаем сохранение конфигурации")
        return true
    end
end

-- И затем использовать:
local success, err = pcall(safeSaveConfig)
if not success then
    print("Ошибка при сохранении конфигурации: " .. tostring(err))
    -- Продолжаем выполнение, так как это не критично для форматирования
	    print("\n✅ Форматирование завершено!")
        print("🔧 Перезапустите ПК")
end
